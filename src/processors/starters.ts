...
export default async function processor() {
  ...
  const starters = db.prepare(`SELECT * FROM Starters`).all();
  ...
  if (starters.length) {
    ...
    db.prepare(`DELETE FROM Starters`).run();
  }
  ...
  const insertStarter = db.prepare(`INSERT INTO Starters(PokemonId) VALUES(?)`);
